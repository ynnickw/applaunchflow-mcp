...
- prepare_screenshot_styles returns the interactive V1/V2 picker in the same tool result. Stop after calling it and let the user choose; do not call apply_screenshot_style yourself. Use render_screenshot_picker only to reopen an already prepared picker. If the host cannot display the widget, provide the returned galleryUrl.
...
- prepare_social_graphics_styles returns the interactive picker in the same tool result. Stop after calling it and let the user choose; do not call apply_social_graphics_style yourself. Use render_social_graphics_picker only to reopen an already prepared picker. If the host cannot display it, provide the exact returned galleryUrl.
...
- generate_promo_video returns all three concepts in the interactive picker in the same tool result. Stop after calling it and let the user choose; do not call apply_promo_video_candidate yourself. Use render_promo_video_picker only to reopen already prepared candidates. If the host cannot display it, provide the exact returned pickerUrl.
