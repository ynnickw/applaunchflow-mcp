...
test("URL opening skips stateless hosted elicitation but preserves local behavior", async (t) => {
  ...
  const request = t.mock.method(server.server, "request", async () => ({
    action: "accept",
  }));
  try {
    process.env.APPLAUNCHFLOW_MCP_REMOTE = "1";
    assert.equal(
      await openUrl(server, "https://example.com/editor", "Open editor"),
      false,
    );
...
test("diagnostic categories never echo arbitrary errors or method names", () => {
  ...
  assert.equal(
    errorCategory({ name: secret, code: secret, message: secret }),
    "unknown",
  );
  assert.equal(
    errorCategory({ status: 503, message: secret }),
    "upstream_error",
  );
  assert.equal(errorCategory({ status: 429 }), "rate_limited");
  assert.equal(
    errorCategory(new DOMException(secret, "AbortError")),
    "cancelled",
  );
  assert.equal(
    toolErrorCategory(fail(new DOMException(secret, "AbortError"))),
    "cancelled",
  );
  assert.equal(
    toolErrorCategory(fail(new DOMException(secret, "TimeoutError"))),
    "timeout",
  );
  assert.equal(
    protocolErrorCategory(
      new Error(`Bad Request: Unsupported protocol version: ${secret}`),
    ),
...
test("hosted reliability regressions over real HTTP", async (t) => {
  ...
  const post = (body: unknown, requestId: string, extraHeaders = {}) =>
    fetch(`${baseUrl}/mcp`, {
      method: "POST",
      headers: { ...headers, "x-request-id": requestId, ...extraHeaders },
      body: JSON.stringify(body),
    });
  const call = (name: string, requestId: string, args = {}) =>
    post(
      {
        jsonrpc: "2.0",
        id: requestId,
  ...
  try {
    await t.test(
      "auth outages return retryable 503 without a credential challenge",
      async () => {
    ...
    await t.test(
      "an elicitation-capable client receives the editor URL without a browser-opening timeout",
      async () => {
        const client = new Client(
          { name: "reliability-test", version: "1.0.0" },
    ...
    await t.test(
      "protocol failures have safe distinct categories",
      async () => {
    ...
    await t.test(
      "guard rejections return stable correctable error codes",
      async () => {
        const transformResponse = await call(
          "transform_layout",
          "guard-transform_layout",
          {
            generationId: "00000000-0000-4000-8000-000000000001",
            language: "en",
            operations: [
        ...
        assert.match(await transformResponse.text(), /"isError":true/);
        assert.equal(
          entries.find(
            (entry) =>
              entry.event === "mcp_tool" &&
              entry.requestId === "guard-transform_layout",
          )?.errorCategory,
        ...
        const pathResponse = await call(
          "upload_screenshots",
          "guard-upload_screenshots",
          {
            projectId: "00000000-0000-4000-8000-000000000001",
            deviceType: "mobile",
            platform: "ios",
            sources: [{ path: "/secret-local-file.png" }],
          },
        );
        assert.equal(pathResponse.status, 200);
        assert.match(await pathResponse.text(), /url or base64/i);
        assert.equal(
          entries.some(
            (entry) =>
              entry.event === "mcp_tool" &&
              entry.requestId === "guard-upload_screenshots",
          ),
          false,
        );
      },
    ...
    await t.test(
      "stateless HTTP text-only reads authorize only matching layout edits",
      async () => {
        ...
        const listing = await decode(
          await call("get_layout", "receipt-list", {
            generationId: target.generationId,
          }),
        );
        assert.equal(
          listing.structuredContent.data.readBeforeEditSatisfied,
          false,
        );
        const read = await decode(
          await call("get_layout", "receipt-read", target),
        );
        ...
        for (const invalid of [
          ...
          const result = await decode(
            await call("transform_layout", "receipt-rejected", invalid),
          );
          assert.equal(result.isError, true);
        }
        assert.equal(layoutWrites.length, 0);
        const saved = await decode(
          await call("transform_layout", "receipt-accepted", edit),
        );
        assert.notEqual(saved.isError, true);
        assert.equal(layoutWrites.length, 1);
        assert.equal(
          JSON.stringify(layoutWrites).includes(data.readReceipt),
          false,
        );
        assert.equal(
          rawLogs.some((line) => line.includes(data.readReceipt)),
          false,
        );
      },
    );
    ...
    await t.test(
      "Cursor SDK validates widget responses against advertised output schemas",
      async () => {
        ...
        const transport = new StreamableHTTPClientTransport(
          ...
          {
            requestInit: {
              headers: { ...headers, "user-agent": "Cursor/3.19.13" },
            },
          },
        );
        try {
          ...
          for (const tool of tools) {
            const outputSchema = tool.outputSchema as
              | {
                  properties?: { widgetDataJson?: unknown };
                  required?: string[];
                }
              | undefined;
            assert.deepEqual(
              outputSchema?.properties?.widgetDataJson,
              { type: "string" },
              tool.name,
            );
            assert.equal(
              outputSchema?.required?.includes("widgetDataJson"),
              false,
              tool.name,
            );
          }
    ...
    await t.test(
      "only Cursor HTTP responses mirror widget metadata",
      async () => {
        for (const userAgent of ["Cursor/3.19.13", "Claude", "ChatGPT"]) {
          ...
          if (userAgent.startsWith("Cursor/")) {
            assert.deepEqual(
              JSON.parse(result.structuredContent.widgetDataJson).projectList,
              result._meta.projectList,
            );
          } else
            assert.equal(result.structuredContent.widgetDataJson, undefined);
        }
      },
    );

    await t.test(
      "concurrent tool logs retain their own HTTP request IDs",
      async () => {
        const responses = await Promise.all([
          call("list_projects", "correlation-success"),
          call("get_project", "correlation-error", {
            projectId: "00000000-0000-4000-8000-000000000001",
          }),
        ]);
        ...
        assert.equal(
          entries.find(
            (entry) =>
              entry.event === "mcp_tool" &&
              entry.requestId === "correlation-error",
          )?.errorCategory,
          "upstream_error",
        );
      },
    ...
    await t.test(
      "disconnected requests produce one aborted access log",
      { timeout: 2000 },
      async () => {
        const started = new Promise<void>((resolve) => {
          onProjectRequest = resolve;
        });
        ...
        const request = fetch(`${baseUrl}/mcp`, {
          method: "POST",
          headers: { ...headers, "x-request-id": "aborted-request" },
        ...
        const matching = entries.filter(
          (entry) =>
            entry.event === "mcp_http_request" &&
            entry.requestId === "aborted-request",
        );
        assert.equal(matching.length, 1);
    ...
    assert.equal(
      rawLogs.some((line) => line.includes("secret-")),
      false,
    );
  } finally {
    await close(hosted);
